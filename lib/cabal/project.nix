...
let
  ...
  cabalSystem = builtins.replaceStrings [ "darwin" ] [ "osx" ]
    builtins.currentSystem; # cabal's convention

  # See notes about LD_LIBRARY_PATH below
  localLdLibPath =
    "$(pwd)/dist-newstyle/build/${cabalSystem}/ghc-${ghcVersion}/${projectName}-${projectVersion}/noopt/build";
  ...
  ldEnvVar =
    if builtins.currentSystem == "x86_64-darwin" then
      "DYLD_LIBRARY_PATH=${localLdLibPath}"
    else
      "LD_LIBRARY_PATH=${localLdLibPath}";
  ...
  projectApps =
    (map runAppScript (builtins.attrNames (earnestProject.components.exes)));
in
{
